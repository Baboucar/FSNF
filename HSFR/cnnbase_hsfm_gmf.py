...
class Model(nn.Module):
    ...
    def __init__(self, user_count, item_count):
        ...
        self.kernel_size = 2
        self.strides = 2
        self.in_layer = nn.Conv2d(3, self.channel_size, self.kernel_size, stride=self.strides)
        self.sec_layer = nn.Conv2d(self.channel_size, self.channel_size, self.kernel_size,
                                   stride=self.strides)
        self.trd_layer = nn.Conv2d(self.channel_size, self.channel_size, self.kernel_size,
                                   stride=self.strides)
    ...
    def forward(self, user_ids, item_ids):
        # GMF
        user_ids, item_ids = torch.div(user_ids, 2, rounding_mode="trunc").cuda(), torch.div(item_ids, 2, rounding_mode="trunc").cuda()
