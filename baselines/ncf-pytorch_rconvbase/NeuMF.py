...
parser.add_argument("--data_path", type=str,
                    default="/home/user/Music/HSFR/baselines/ncf-pytorch_rconvbase/data")
parser.add_argument("--model_path", type=str,
                    default="/home/user/Music/HSFR/baselines/ncf-pytorch_rconvbase/model")
...
if __name__ == "__main__":
    ...
    if args.use_pretrained:
        ...
        GMF_model = GMF(user_num, item_num, args.embedding_dim_GMF, args.dropout)
        GMF_model.load_state_dict(torch.load(GMF_model_path))

        MLP_model = MLP(user_num, item_num, args.embedding_dim_MLP, args.hidden_layer_MLP, args.dropout)
    ...
    for epoch in range(args.epochs):
        ...
        for user, item, label in train_loader:
            ...
            count += 1
            # print("view-->", loss.item(), length)
        # print("view-->", torch.mean(torch.tensor(totalloss)), torch.sum(torch.tensor(totalloss))/length)
        best_tloss = torch.mean(torch.tensor(totalloss))
        model.eval()
        HR, NDCG = evaluate.metrics(model, test_loader, args.top_k, args.device)
        vloss = evaluate.validation_MAEloss(model, test_loader, args.device)  ############################
        best_vloss = vloss
        writer.add_scalar("ml-1mtrainMAEloss@NeuMF", best_tloss, epoch)
        writer.add_scalar("ml-1mvalidationMAEloss@NeuMF", best_vloss, epoch)

        writer.add_scalar("ml-1mHRMAW@NeuMF", HR, epoch)
        writer.add_scalar("ml-1mNDCGMAE@NeuMF", NDCG, epoch)
...
print("End. Best epoch {:03d}: HR = {:.4f}, NDCG = {:.4f}, best_tloss = {:.4f}, best_vloss = {:.4f}".format(best_epoch,
                                                                                                            best_hr,
                                                                                                            best_ndcg,
                                                                                                            best_tloss,
                                                                                                            best_vloss))
