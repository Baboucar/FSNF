...
class DMF(nn.Module):
    def __init__(self, userLayers, itemLayers, userMatrix, itemMatrix):
        ...
        assert userLayers[-1] == itemLayers[
            -1], "The last layer size of 'userLayers' and 'itemLayers' must be the same!"
...
if __name__ == "__main__":
    ...
    print(
        f"Load data: #user={nUsers}, #item={nItems}, #train={train.nnz}, #test={len(testRatings)} [{time.time() - t1:.1f}s]")
    ...
    for epoch in range(args.epochs):
        ...
        for ui, ii, lbl in ldr:
            if isCuda:
                ui, ii, lbl = ui.long().cuda(), ii.long().cuda(), lbl.cuda()
        ...
        hits, ndcgs = evaluate_model(model, testRatings, testNegatives, topK, num_thread=1)
        hr, ndcg = np.array(hits).mean(), np.array(ndcgs).mean()
        writer.add_scalar('ml1mRMSEHR@DMF', hr, epoch)
        writer.add_scalar('ml1mRMSENDCG@DMF', ndcg, epoch)
        writer.add_scalar("ml-1mRMSEloss", losses.avg)
        print(f"Epoch {epoch + 1}: HR={hr:.4f}, NDCG={ndcg:.4f} [{time.time() - t1:.1f}s]")
        if hr > bestHr:
            bestHr, bestNdcg, bestEpoch = hr, ndcg, epoch
            torch.save(model.state_dict(), modelPath)

    print(f"Best epoch {bestEpoch + 1}:  HR={bestHr:.4f}, NDCG={bestNdcg:.4f},best_loss={best_loss:.4f}")
