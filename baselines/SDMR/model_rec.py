...
class Rec(ModelBased):
    ...
    def _initialize(self, interactions, max_seq_len=-1):
        ...
        self._loss_func = nn.L1Loss().cuda()
    ...
    def fit(self, interactions, topN=10,
        ...
        if self._has_params():
            for epoch in range(self._n_iters):
                ...
                writer.add_scalar("ml-1mHRMAE@SDMR", hits, epoch)
                writer.add_scalar("ml-1mMAENDCG@SDMR", ndcgs, epoch)
                t2 = time.time()
                eval_time = t2 - t1
                ...
                writer.add_scalar("sdmr_rMAEloss", epoch_loss, epoch)
                if hits > best_hit or (hits == best_hit and ndcgs >= best_ndcg):
                    best_hit, best_ndcg, best_epoch, best_loss = hits, ndcgs, epoch, epoch_loss
                    if args.out:
            ...
            print('Best result: '
                  '| test hits@%d = %.4f | test ndcg@%d = %.4f | epoch = %d ' % (
                  topN, best_hit, topN, best_ndcg,
                  best_epoch))
